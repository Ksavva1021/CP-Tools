...
class PolarimetricA1:
    ...
    def BWIGML(self,S,M,G,m1,m2,L):
        ...
        out = np.complex(MSQ,0)/np.complex(MSQ-S,-WGS)
        return out
    ...
    def FA1A1P(self, XMSQ):
        ...
        FG1 = GG1*GF
        FG2 = GG2*GF
        F1 = np.complex(-XM1SQ,0.0)/np.complex(XMSQ-XM1SQ,FG1)
        F2 = np.complex(-XM2SQ,0.0)/np.complex(XMSQ-XM2SQ,FG2)
        FA1A1P = F1+BET*F2
        return FA1A1P
    ...
    def Conjugate(self, a):
        return np.complex(a.real, -a.imag)
    ...
    def PVC(self):
        ...
        F1 = np.complex(self.COEF1)*self.F3PI(1, a1.M2(), s1, s2)
        F2 = np.complex(self.COEF2)*self.F3PI(2, a1.M2(), s2, s1)
        F3 = np.complex(self.COEF3)*self.F3PI(3, a1.M2(), s3, s1)
        
        HADCUR = []
        HADCURC = []
        
        HADCUR.append(np.complex(vec1.E())*F1 + np.complex(vec2.E())*F2 + np.complex(vec3.E())*F3) # E
        HADCUR.append(np.complex(vec1.Px())*F1 + np.complex(vec2.Px())*F2 + np.complex(vec3.Px())*F3)  
        HADCUR.append(np.complex(vec1.Py())*F1 + np.complex(vec2.Py())*F2 + np.complex(vec3.Py())*F3)
        HADCUR.append(np.complex(vec1.Pz())*F1 + np.complex(vec2.Pz())*F2 + np.complex(vec3.Pz())*F3)
        
        HADCURC.append(self.Conjugate(np.complex(vec1.E())*F1 + np.complex(vec2.E())*F2 + np.complex(vec3.E())*F3)) # E
        HADCURC.append(self.Conjugate(np.complex(vec1.Px())*F1 + np.complex(vec2.Px())*F2 + np.complex(vec3.Px())*F3))
        HADCURC.append(self.Conjugate(np.complex(vec1.Py())*F1 + np.complex(vec2.Py())*F2 + np.complex(vec3.Py())*F3))
        HADCURC.append(self.Conjugate(np.complex(vec1.Pz())*F1 + np.complex(vec2.Pz())*F2 + np.complex(vec3.Pz())*F3))
